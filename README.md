...
This repository contains the general robot-retargeting toolchain. It supports SMPL-X and related human-motion inputs, robot-specific skeleton/URDF/MJCF assets, debug visualization, and downstream training exports. Robot assets remain separate from body-model and private dataset storage.

Run the supplied scripts with the local Python environment and explicit config/input/output paths. Before publishing a retargeted clip, verify the robot model, joint mapping, mesh availability, quaternion convention, time base, joint limits, contacts, and output schema. Large body models and private datasets require a separately licensed artifact or release.
