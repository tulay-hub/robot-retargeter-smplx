...
SKELETON_CONFIG="${SKELETON_CONFIG:-config/robot/${ORIGIN_ROBOT}.yaml}"
KEYPOINTS_NAME="${KEYPOINTS_NAME:-$(basename "${ROBOT_MOTION_FILE}" .csv)}"
...
for idx in "${!VIS_ROBOT_ARR[@]}"; do
  ...
  echo "[1/3] robot_replay (no viewer)"
  "${PYTHON_BIN}" scripts/robot_replay.py \
    --no-viewer \
    --motion-file "${ROBOT_MOTION_FILE}" \
    --source-robot-config "${SKELETON_CONFIG}" \
...
# robot motion (KEYPOINTS_NAME), or from the keypoints_path of the first robot.
if [[ -n "${KEYPOINTS_NAME}" ]]; then
  MOTION_NAME="${KEYPOINTS_NAME}_from_${ORIGIN_ROBOT}"
else
  ...
  MOTION_NAME="${KEYPOINT_STEM}_from_${ORIGIN_ROBOT}"
fi
