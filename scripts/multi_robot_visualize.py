...
def build_combined_spec(robots: list[str], add_ground: bool = True) -> mujoco.MjSpec:
...
def main() -> None:
    ...
    parser.add_argument(
        "--no_ground",
        action="store_true",
        help="Skip the built-in gray grid ground; use the robot XML's own floor if it has one",
    )
    ...
    spec = build_combined_spec(robots, add_ground=not args.no_ground)
    model = spec.compile()
